...
private theorem slowGraphSecondLegAmplitudeGerm :
    EqModPow 8
      (fun ε : ℝ ↦
  ...
  have hw₁Cont : ContinuousAt w₁ 0 := by
    ...
    have hfun :
        (fun ε ↦ ε * L ε * Q ε - 2 * H ε * U ε) =
          ((fun ε : ℝ ↦ ε) * L) * Q - ((fun _ : ℝ ↦ (2 : ℝ)) * H) * U := by
      funext ε
      rfl
    rw [hfun]
...
/-- Along the polynomial slow-graph path, the normalized amplitude equals
`1 - (13 / 2) * ε ^ 4 + (116 / 5) * ε ^ 6 - (976 / 5) * ε ^ 7` up to
`O(ε ^ 8)`. -/
theorem slowGraphAmplitudeRemainder :
    (fun ε : ℝ ↦
...
theorem amplitudeRemainderOfSlowGraphJets (p h : ℝ → ℝ)
    ...
    (fun ε : ℝ ↦
      (observableMap (ε, p ε, h ε)).amplitudeRatio -
        (1 - (13 / 2) * ε ^ 4 + (116 / 5) * ε ^ 6 - (976 / 5) * ε ^ 7)) =O[𝓝 0]
      (fun ε : ℝ ↦ ε ^ 8) := by
  -- Route correction: bypass the unfinished divided-difference chain and compare the
  -- second-leg amplitude directly with its slow-graph specialization.
  ...
  have hsum := hdiff.add slowGraphAmplitudeRemainder
...
/-- Along the polynomial slow-graph path, the normalized amplitude has
order-seven jet
`1 - (13 / 2) * ε ^ 4 + (116 / 5) * ε ^ 6 - (976 / 5) * ε ^ 7`. -/
theorem slowGraphAmplitudeJet :
    FiniteTaylorJet.ofFunction ℝ 7
        (fun ε : ℝ ↦
          (observableMap (slowGraphJetPath ε)).amplitudeRatio) 0 =
      FiniteTaylorJet.ofFunction ℝ 7
        (fun ε : ℝ ↦
          1 - (13 / 2) * ε ^ 4 + (116 / 5) * ε ^ 6 - (976 / 5) * ε ^ 7) 0 := by
  have hpolynomialPath : ContDiffAt ℝ 7
      (fun ε : ℝ ↦
  ...
  have hpathEq : slowGraphJetPath =
    ...
    funext ε
    exact slowGraphJetPath_apply ε
  have hpath : ContDiffAt ℝ 7 slowGraphJetPath 0 := by
    simpa only [hpathEq] using hpolynomialPath
  have hpathBase : slowGraphJetPath 0 = ((0, 2, 1) : ℝ × ℝ × ℝ) := by
    rw [slowGraphJetPath_apply]
    norm_num
  have hamplitudeBase := amplitudeRatio_contDiffAt 7
  rw [← hpathBase] at hamplitudeBase
  have hamplitude : ContDiffAt ℝ 7
      (fun ε : ℝ ↦ (observableMap (slowGraphJetPath ε)).amplitudeRatio) 0 := by
    exact hamplitudeBase.comp 0 hpath
  have hpolynomial : ContDiffAt ℝ 7
      (fun ε : ℝ ↦
        1 - (13 / 2) * ε ^ 4 + (116 / 5) * ε ^ 6 - (976 / 5) * ε ^ 7) 0 := by
    fun_prop
  apply FiniteTaylorJet.ofFunction_eq_of_sub_isBigO_succ hamplitude hpolynomial
  simpa only [zero_add, Nat.reduceAdd] using slowGraphAmplitudeRemainder
