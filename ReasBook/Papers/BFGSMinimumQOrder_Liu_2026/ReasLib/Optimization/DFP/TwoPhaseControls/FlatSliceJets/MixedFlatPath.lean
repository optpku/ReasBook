...
public import ReasLib.Analysis.Asymptotics.ContinuousMultilinearMap
...
private theorem diagonal_two_scale_cubic_quartic
    (A : E [×2]→L[ℝ] F) (u v₃ v₄ : E)
    (hcross : A ![u, v₃] + A ![v₃, u] = 0) (ε : ℝ) :
    A (fun _ : Fin 2 =>
          ε • u + (ε ^ 3 • v₃ + ε ^ 4 • v₄)) -
        A (fun _ : Fin 2 => ε • u) -
  ...
  calc
    (ε * ε ^ 3) • A ![u, v₃] + (ε * ε ^ 4) • A ![u, v₄] +
        ((ε ^ 3 * ε) • A ![v₃, u] + (ε ^ 4 * ε) • A ![v₄, u]) =
      ε ^ 4 • (A ![u, v₃] + A ![v₃, u]) +
        ε ^ 5 • (A ![u, v₄] + A ![v₄, u]) := by
      module
    _ = ε ^ 5 • (A ![u, v₄] + A ![v₄, u]) := by
      rw [hcross, smul_zero, zero_add]

/-- For a formal power series, the mixed part of its degree-four partial sum along
`ε • u + ε³ • v₃ + ε⁴ • v₄` is order five when the quadratic cross coefficient
in the `(u, v₃)` directions vanishes. -/
theorem partialSum_five_mixed_cubic_quartic_isBigO
    (p : FormalMultilinearSeries ℝ E F) (u v₃ v₄ : E)
    (hcross : p 2 ![u, v₃] + p 2 ![v₃, u] = 0) :
    (fun ε : ℝ =>
      let x := ε • u
      let y := ε ^ 3 • v₃ + ε ^ 4 • v₄
      p.partialSum 5 (x + y) - p.partialSum 5 x -
        p.partialSum 5 y + p.partialSum 5 0) =O[𝓝 0]
      (fun ε : ℝ => ε ^ 5) := by
  let x : ℝ → E := fun ε => ε • u
  ...
  have hterm₂ :
      (fun ε : ℝ =>
        p 2 (fun _ : Fin 2 => x ε + y ε) - p 2 (fun _ : Fin 2 => x ε) -
          p 2 (fun _ : Fin 2 => y ε) + p 2 (fun _ : Fin 2 => 0)) =O[𝓝 0]
        (fun ε : ℝ => ε ^ 5) := by
    ...
    · filter_upwards
      ...
      exact (diagonal_two_scale_cubic_quartic (p 2) u v₃ v₄ hcross ε).symm
    · filter_upwards
      intro ε
      simp
  have hdiagY₃ :
...
/-- An analytic map has no mixed terms through degree four along a linear direction
and a cubic-quartic perturbation if its quadratic cross derivative in the linear and
cubic directions vanishes. -/
theorem analytic_mixed_cubic_quartic_isBigO [CompleteSpace F]
    (f : E → F) (a u v₃ v₄ : E) (hf : AnalyticAt ℝ f a)
    (hcross : iteratedFDeriv ℝ 2 f a ![u, v₃] = 0) :
    (fun ε : ℝ =>
      let x := ε • u
      let y := ε ^ 3 • v₃ + ε ^ 4 • v₄
      f (a + (x + y)) - f (a + x) - f (a + y) + f a) =O[𝓝 0]
      (fun ε : ℝ => ε ^ 5) := by
  obtain ⟨p, hpAt⟩ := hf
  ...
  have hcrossCoeff : p 2 ![u, v₃] + p 2 ![v₃, u] = 0 := by
    rw [← hiter]
    exact hcross
  let x : ℝ → E := fun ε => ε • u
  let y : ℝ → E := fun ε => ε ^ 3 • v₃ + ε ^ 4 • v₄
  ...
  have hpoly :
      (fun ε : ℝ =>
        p.partialSum 5 (x ε + y ε) - p.partialSum 5 (x ε) -
          p.partialSum 5 (y ε) + p.partialSum 5 0) =O[𝓝 0]
        (fun ε : ℝ => ε ^ 5) := by
    simpa only [x, y] using
      partialSum_five_mixed_cubic_quartic_isBigO p u v₃ v₄ hcrossCoeff
