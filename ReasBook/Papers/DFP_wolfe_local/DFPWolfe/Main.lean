...
/-- The paper-range strong-Wolfe counterexample is available in every
...
/-- Main theorem: for every pair in the paper's Wolfe range, the
...
/-- The preceding negative convergence theorem is uniform over all
...
/-- The same paper-range counterexample refutes the level-set version
...
/-- The level-set global-convergence predicate is false for every
...
/-- A named positive predicate for convergence under the paper's
...
/-- A direct negation of the positive convergence predicate
...
/-- The level-set formulation also has a direct paper-range
negation predicate. -/
...
/-- The level-set paper-range convergence predicate is false. -/
...
/-- The identity-initialized strong-Wolfe corollary is exposed with
...
/-- For every pair in the paper's coefficient range and every
...
/-- Theorem 3 (`thm:planar-convergence`): arbitrary initial
...
theorem main_planarWeakWolfeConvergence
  ...
  have hstrict := (hf.strongConvexOnOfHessianLowerBound f μ hμ hHessian).strictConvexOn hμ
  refine ⟨xstar, hresult.1, ?_, hresult.2⟩
  intro z hz
  have hminz : IsMinOn f Set.univ z := fun w _ ↦ hz.trans (hresult.1 w)
  have hminstar : IsMinOn f Set.univ xstar := fun w _ ↦ hresult.1 w
  exact hstrict.eq_of_isMinOn hminz hminstar (Set.mem_univ z) (Set.mem_univ xstar)
